...
export default function Footer() {
  ...
  useEffect(() => {
    const fmt = new Intl.DateTimeFormat("en-GB", {
      hour: "2-digit", minute: "2-digit", second: "2-digit", timeZone: siteData.meta.timezone
    });
  ...
  return (
    <footer className="bg-gray border-t border-line px-6 lg:px-14 pt-8 pb-10 flex flex-wrap gap-y-4 gap-x-9 items-center justify-between text-xs text-muted relative z-50">
      <div>
        © {new Date().getFullYear()} · {siteData.meta.location} · <span>{time}</span> WIB
      </div>
      <div className="flex flex-wrap gap-5">
        {siteData.socials.map((s, i) => (
